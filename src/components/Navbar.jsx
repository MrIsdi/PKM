...
function MobileView({ menu, display }) {
  return (
    <React.Fragment>
      <div
        className={`${display} basis-1/2 justify-evenly flex-col text-center delay-300`}
      >
        {menu.map((data, i) => (
          <Link
            to={data}
            ...
            duration={500}
            key={i}
            className={`font-medium lg:py-0 py-3 text-white`}
          >
            {data}
          </Link>
        ))}
...
function Navbar() {
  ...
  return (
    <React.Fragment>
      <nav
        className={`lg:container lg:px-0 px-[16px] flex mx-auto justify-between items-center`}
      >
      ...
      {burger ? (
        <MobileView menu={menu} display={"flex"} />
      ) : (
        <MobileView menu={menu} display={"hidden"} />
      )}
    </React.Fragment>
  )
}
