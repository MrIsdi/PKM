...
function MobileView({menu, display}){
    return(
        <React.Fragment>
            <div className={`${display} basis-1/2 justify-evenly flex-col text-center`}>
                {
                    menu.map((data, i)=>(
                        <a href="#" key={i} className={`font-medium lg:py-0 py-3`}>{data}</a>
                    ))
                }
            </div>
        </React.Fragment>
        ...
        <React.Fragment>
            <nav className={`lg:container lg:px-0 px-[16px] flex mx-auto justify-between items-center`}>
                <div className={`flex items-center`}>
                    ...
                    <div>
                        <p className="text-xs">PKM - KC</p>
                        <p className='text-2xl font-bold'>CV TOMATO</p>
                        <p className="text-xs">Universitas Andalas</p>
                    </div>
                </div>
                <div className={`lg:flex basis-1/2 justify-evenly hidden`}>
                    {
                        menu.map((data, i)=>(
                            <a href="#" key={i} className={`font-medium`}>{data}</a>
                        ))
                    }
                </div>
